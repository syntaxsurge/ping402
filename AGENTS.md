...
- Solana (Token-2022) badge mint: `pnpm solana:create-badge-mint`
- x402 buyer demo: `pnpm x402:buyer`
- Lint: `pnpm lint`
...
- `GET /r/[messageId]` — public receipt page (payment + badge tx links)
...
- `POST /api/solana-pay/ping-intents/[intentId]/consume` — consumes an intent (creates the message + optional badge mint)
...
Middleware:
- `src/middleware.ts` attaches `x-request-id`, applies security headers on `/api/**` requests/responses (excluding `/api/health`), and optionally enforces Upstash-backed rate limits when configured
...
- **x402 + Solana payments**: `src/app/api/ping/send/route.ts`, `src/app/api/auth/verify/route.ts`, and `src/app/api/x402/demo/route.ts` wrap handlers with `withX402` from `@x402/next`, using the Solana `exact` scheme registered in `src/lib/x402/server.ts` (including `bazaarResourceServerExtension`). Payment proofs arrive in the `PAYMENT-SIGNATURE` header and settlement receipts arrive in `PAYMENT-RESPONSE`. Paywalled routes declare Bazaar discovery schemas via `declareDiscoveryExtension` in `@x402/extensions/bazaar`. `src/lib/solana/badges.ts` mints an optional Token-2022 non-transferable supporter badge after a successful ping and stores the badge mint transaction signature (`badgeTxSig`) on the message.
- **Bazaar discovery**: `src/app/api/x402/discovery/resources/route.ts` proxies facilitator discovery listings (including CDP auth) for UI/debug tooling and buyer validation.
- **Buyer validation**: `scripts/x402/buyer.ts` demonstrates `discover → pay → request → response` using `@x402/fetch` + `registerExactSvmScheme` against the deployed app URL.
- **Solana Pay checkout**: `src/app/api/solana-pay/ping-intents/*` creates a Solana Pay transfer request URL based on the same x402-derived payment requirements (mint + amount + payTo), tracks the payment intent in Convex, confirms payment by finding the `reference` signature and validating the on-chain token balance delta, then consumes the intent to create the message and mint the supporter badge.
...
- **Environment variables**: `.env.example` documents required variables; Convex writes `NEXT_PUBLIC_CONVEX_URL` + `CONVEX_DEPLOYMENT` to `.env.local`. `NEXT_PUBLIC_SITE_URL` configures absolute URLs used by metadata routes (falls back to `VERCEL_URL` or `http://localhost:3000`). x402 uses `X402_NETWORK` (Solana CAIP-2 chain id) plus `X402_FACILITATOR_URL` (testnet or CDP facilitator). `SOLANA_RPC_URL` and `SOLANA_WS_URL` optionally override the Solana RPC endpoints (defaults derive from `X402_NETWORK`). New handle claims require `PING402_CLAIM_PAY_TO_WALLET` (claim fee recipient). `CDP_API_KEY_ID` and `CDP_API_KEY_SECRET` are required when `X402_FACILITATOR_URL` points to the CDP facilitator. Creator sessions require `PING402_JWT_SECRET`. Token-2022 supporter badges use `PING402_BADGE_MINT` plus `PING402_BADGE_AUTHORITY_SECRET_KEY` (or `PING402_BADGE_AUTHORITY_KEYPAIR_PATH` for local dev). Edge rate limiting uses `UPSTASH_REDIS_REST_URL` and `UPSTASH_REDIS_REST_TOKEN` when configured. Buyer demo tooling uses `NEXT_PUBLIC_SITE_URL`/`VERCEL_URL` as the base URL and requires `PING402_BUYER_SECRET_KEY` (or `PING402_BUYER_KEYPAIR_PATH`).
